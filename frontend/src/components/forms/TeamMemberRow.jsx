...
const TeamMemberRow = ({
  ...
  return (
      <div className="team-member-row">
        ...
        <input {...register(fieldKey + '.name')}
               placeholder="Nombre"
               type="text" />
        <input {...register(fieldKey + '.nickname')}
               placeholder="Alias"
               type="text" />
        <input {...register(fieldKey + '.jerseyNo')}
               placeholder="No. Jersey"
               type="number"
               step={1}
               min={0}
               defaultValue={0} />
        <button className="plain" onClick={onRemove}><IconTrash className="icon-trash" /></button>
      </div>
  );
};
